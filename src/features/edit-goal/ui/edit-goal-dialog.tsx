...
export const EditGoalDialog = ({
  ...
  const { data: goal, isLoading: isGoalLoading } = useGetGoalByIdQuery(
    numericGoalId,
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      setRootError(
        getApiErrorMessage(error, t("savingsPage.editGoal.errors.delete")),
      );
    }
  };
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="flex max-h-[min(90vh,640px)] max-w-lg flex-col gap-0 p-0 sm:max-w-xl">
        <DialogHeader className="shrink-0 px-6 pt-6 pb-4 sm:px-8">
          ...
          <DialogDescription>
            {t("savingsPage.editGoal.description")}
          </DialogDescription>
        </DialogHeader>
